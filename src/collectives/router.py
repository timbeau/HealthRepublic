...
def join_collective_demo(
    collective_id: int,
    user_id: int,
    db: Session = Depends(get_db),
):
    """
    DEMO: join a collective by specifying user_id explicitly.
    ...
    membership = services.join_collective(db=db, user=user, collective_id=collective_id)
    return membership


@router.post("/{collective_id}/leave")
def leave_collective_demo(
    collective_id: int,
    user_id: int,
    db: Session = Depends(get_db),
):
    """
    DEMO: leave a collective by specifying user_id explicitly.
    """
    user = db.query(User).filter(User.id == user_id).first()
    if not user:
        raise HTTPException(
    ...
    removed = services.leave_collective(db=db, user=user, collective_id=collective_id)
    if not removed:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
    ...
    return {"message": "Left collective", "collective_id": collective_id, "user_id": user.id}
